...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#fdfbf7] text-stone-800 font-sans selection:bg-orange-100">
      ...
      {!isAuthPage && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-stone-100 py-3 px-6 z-40 max-w-md mx-auto shadow-[0_-4px_20px_rgba(0,0,0,0.02)]" style={{ touchAction: 'auto' }}>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all duration-300 ${isActive ? 'text-orange-500 -translate-y-1' : 'text-stone-400 hover:text-stone-500'}`}
